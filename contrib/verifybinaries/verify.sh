...
###   This script attempts to download the signature file SHA256SUMS.asc from softwarechain.org
...
WORKINGDIR="/tmp/softcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://softwarechain.org/bin/"
VERSIONPREFIX="softcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'softcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SoftCoin Core binary release signing key installed?"
   fi
