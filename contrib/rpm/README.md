...
The RPM spec file provided here is for SoftCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://softwarechain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[SoftCoin git repository](https://github.com/softcoin/softcoin) but are not part of
...
Currently only `Source10` is used, for the example `softcoin.conf` file.
...
in the softcoin git repository. At present only an SVG version of the SoftCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the softcoin
...
needed when 0.12.1 is released, a proper fix is already in the SoftCoin git
master branch.
...
idea to build SoftCoin Core against a static version of that library compiled
...
The `softcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `softcoin-server` package. At this point, most RPM based
...
The `softcoin-server` package should have SELinux support. How to properly do
...
SoftCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/softcoin/). He has been
packaging SoftCoin for Fedora at least since 2012.
...
4. I renamed the `softcoin` package that contains the Qt GUI to `softcoin-core` as
that appears to be how the general population refers to it, in contrast to
`softcoin-xt` or `softcoin-classic`. I wanted to make sure the general population
