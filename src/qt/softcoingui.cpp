...
const std::string SoftCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SoftCoinGUI::DEFAULT_WALLET = "~Default";

SoftCoinGUI::SoftCoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void SoftCoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/softcoin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SoftCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and softcoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SoftCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SoftCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a softcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible SoftCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void SoftCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        Q_FOREACH(SoftCoinUnits::Unit u, SoftCoinUnits::availableUnits())
        {
            QAction *menuAction = new QAction(QString(SoftCoinUnits::name(u)), this);
...
void SoftCoinGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void SoftCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        topMenuLogo->setObjectName("softLogo");
        topMenuLogo->move(0,0);
        topMenuLogo->setStyleSheet(
           "#softLogo { border-image: url(:/icons/menu_logo)  0 0 0 0 stretch stretch; border: 0px; }");
        ...
        // ImageButton* softLogo2 = new ImageButton();
        // softLogo2->setFixedSize(64,64);
        // softLogo2->setObjectName("softLogo");
        // softLogo2->setPixmap(pixmap);
        //
        // toolbar->setMovable(false);
        // toolbar->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
        // toolbar->addWidget(softLogo);
        // toolbar->addWidget(softLogo2);
...
void SoftCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool SoftCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void SoftCoinGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool SoftCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SoftCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SoftCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void SoftCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SoftCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SoftCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SoftCoinGUI::optionsClicked()
{
...
void SoftCoinGUI::showDebugWindow()
{
...
void SoftCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SoftCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SoftCoinGUI::openClicked()
{
...
void SoftCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void SoftCoinGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SoftCoinGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SoftCoinGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SoftCoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SoftCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SoftCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SoftCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SoftCoin network", "", count));
    if(walletFrame){
...
void SoftCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to SoftCoin network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to SoftCoin network..."));
...
void SoftCoinGUI::showVotingDialog()
{
...
void SoftCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("SoftCoin"); // default title
...
void SoftCoinGUI::changeEvent(QEvent *e)
{
...
void SoftCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SoftCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SoftCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SoftCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SoftCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SoftCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SoftCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SoftCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SoftCoinGUI::setEncryptionStatus(int status)
{
...
void SoftCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SoftCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SoftCoinGUI::detectShutdown()
{
...
void SoftCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SoftCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(SoftCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SoftCoinGUI::subscribeToCoreSignals()
{
...
void SoftCoinGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<SoftCoinUnits::Unit> units = SoftCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SoftCoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SoftCoinUnits::name(unit)));
    }
...
{
    setText(SoftCoinUnits::name(newUnits));
}
...
void SoftCoinGUI::toggleStaking()
{
...
void SoftCoinGUI::replyFinished(QNetworkReply *reply)
{
...
void SoftCoinGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CSoftCoinAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void SoftCoinGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.softwarechain.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void SoftCoinGUI::updateStakingStatus()
{
    updateWeight();
