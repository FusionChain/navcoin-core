...
UniValue Softnode::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "soft_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue softnodeData;
    softnodeData.setObject();
    softnodeData.pushKV("anondestination", encryptedAddress);
    softnodeData.pushKV("public_key", pubKey);
    softnodeData.pushKV("anonaddress", addrArray);
    softnodeData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    softnodeData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return softnodeData;
  } else {
    throw runtime_error("Unable to send SOFTNode transaction, please try again.");
  }
...
vector<anonServer> Softnode::GetAnonServers() {
  ...
  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one SOFTNode server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid SOFTNode server to send private payments.");
      return returnServers;
  }
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue softnodeData;
        softnodeData.setObject();
        softnodeData.pushKV("min_amount", find_value(data, "min_amount"));
        softnodeData.pushKV("max_amount", find_value(data, "max_amount"));
        softnodeData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return softnodeData;
      }
