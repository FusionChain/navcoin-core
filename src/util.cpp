...
const char * const SOFTCOIN_CONF_FILENAME = "softcoin.conf";
const char * const SOFTCOIN_PID_FILENAME = "softcoin.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SoftCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SoftCoin
    // Mac: ~/Library/Application Support/SoftCoin
    // Unix: ~/.softcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SoftCoin4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SoftCoin4";
#else
    // Unix
    return pathRet / ".softcoin4";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", SOFTCOIN_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No softcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override softcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", SOFTCOIN_PID_FILENAME));
